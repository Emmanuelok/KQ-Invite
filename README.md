...
When Vercel proxies guest submissions to a sign-in-protected Sites backend, set
`OAI_SITES_BYPASS_TOKEN` as a server-only Vercel environment variable. The proxy
forwards it only in the `OAI-Sites-Authorization` request header; never prefix it
with `NEXT_PUBLIC_` or commit its value.
